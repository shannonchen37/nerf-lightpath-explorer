...
OUT=ROOT/"results/d1_official_demo"; RES=256; PRIMARY_SPP=16; SECONDARY_SPP=16
...
def main() -> None:
    ...
    subprocess.run([sys.executable,str(ROOT/"reimpl/run_d1_uv_oracle.py"),str(uv_in),str(uv_out),str(ASSETS/"mesh.obj")],check=True)
    ...
    reference=load_exr(OUT/"official_reference.exr",device)
    ...
    official_mask=torch.load(OUT/"official_spp_256_mask.pt")[...,0].to(device)>.5; interior=center_hit&official_mask
    ...
    (OUT/"visual_parity_metrics.json").write_text(json.dumps(metrics,indent=2)); save_comparisons(reference,ours,interior)
    print(json.dumps(metrics,indent=2)); print("D1 FINAL RENDER: "+metrics["status"])
