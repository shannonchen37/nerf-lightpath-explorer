"""Spatial-GMM/vMF guiding reconstructed for reference-parity validation."""
...
class ReferenceVmfMixture:
    """64-component proposal reconstructed to match the public parity target."""
    ...
    def __init__(self,position:torch.Tensor,std:torch.Tensor,weight:torch.Tensor) -> None:
        if position.shape!=(64,3) or std.shape!=(64,) or weight.shape!=(64,):
            raise ValueError("reference teapot proposal must contain 64 isotropic components")
    ...
    @classmethod
    def from_checkpoint(cls,path:Path,device:torch.device) -> "ReferenceVmfMixture":
    ...
    def sample(self,points:torch.Tensor,uniforms:torch.Tensor) -> VmfSamples:
        ...
        # Retain the reference sampler's reuse of the remapped component variate.
...
# Compatibility alias for historical scripts.
OfficialVmfMixture = ReferenceVmfMixture
