...
ROOT=Path.home()/"nerf_inverse_rendering"; OUT=ROOT/"results/d1_official_demo"
...
def main() -> None:
    ...
    official=torch.from_numpy(np.asarray(env.pdf_direction(it,ds)).astype(np.float32,copy=True)).to(device)
    ours=vmf.pdf(points,directions); pdf_error=(ours-official).abs()
    pdf_relative=pdf_error/official.abs().clamp_min(1e-6)
    ...
    result={"status":"PASS","component_count":64,"checkpoint_vs_official_emitter_max_abs":parameter_error,
            "vmf_pdf_vs_official":{"mae":float(pdf_error.mean()),"max_abs":float(pdf_error.max()),
