"""Generate exact D1 pixel traces, including Gaussian reconstruction samples."""
...
from reimpl.hybrid_teapot.demo_renderer import camera_rays_from_positions,jittered_primary_rays
...
D1=ROOT/"results/d1_official_demo"; OUT=ROOT/"results/d2_pixel_inspector"; TRACES=OUT/"traces"
...
def main() -> None:
    ...
    recorded=load_exr(D1/"ours_final.exr",device); official_mask=torch.load(D1/"official_spp_256_mask.pt")[...,0]>.5

    if args.pixels:
        pixels=[tuple(map(int,p.split(","))) for p in args.pixels]
    else:
        mask=official_mask.cpu().numpy().astype(np.uint8); eroded=torch.from_numpy(__import__("cv2").erode(mask,np.ones((7,7),np.uint8))>0)
        ref=load_exr(D1/"official_reference.exr",device); lum=(ref*LUMA.to(device)).sum(-1).cpu()
