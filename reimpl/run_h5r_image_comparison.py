"""Complete H5R raw image comparison using the existing layered H5 harness."""
...
from reimpl.hybrid_teapot.principled_renderer import render_principled_teapot
...
from reimpl.run_r2_mirror_sphere import save_exr, save_png
from reimpl.run_r3_ggx_sphere import scaled_camera_rays
...
OUT=ROOT/"results/h5_official_equivalence"
REL_EPS=1e-4
...
def masks(official: torch.Tensor, ours: torch.Tensor) -> dict[str,torch.Tensor]:
    kernel=np.ones((5,5),np.uint8)
    union=(official|ours).cpu().numpy().astype(np.uint8)
    inter=(official&ours).cpu().numpy().astype(np.uint8)
    band=(cv2.dilate(union,kernel)-cv2.erode(inter,kernel))>0
    return {"full":torch.ones_like(official),"teapot_interior":official&ours&~torch.from_numpy(band).to(official.device),
            "background":~(official|ours),"silhouette_band":torch.from_numpy(band).to(official.device)}
...
def main() -> None:
    ...
    official_uv=torch.from_numpy(oracle["uv"]).to(device).float()
    if official_uv.shape!=(256*256,2): raise RuntimeError("H5 Mitsuba UV oracle is not the 256x256 camera corpus")
    ...
    for spp in (16,64,256):
        render=render_principled_teapot(emitter,geometry,rays,bsdf,spp,camera_idx=0,uv_override=official_uv,
    ...
    official_mask=torch.from_numpy(np.load(OUT/"official_spp_256_mask.npy")[...,0]>.5).to(device)
    ours_mask=hit.hit.reshape(256,256)
    region_masks=masks(official_mask,ours_mask)
    convergence={"definition":{"relative_epsilon":REL_EPS,"teapot_interior":"intersection minus 2-pixel silhouette band",
                 ...
                 "mask_agreement":{"official_pixels":int(official_mask.sum()),"ours_pixels":int(ours_mask.sum()),
                                   "classification_equal":int((official_mask==ours_mask).sum()),
                                   "mismatch":int((official_mask!=ours_mask).sum())},"spp":{}}
    for spp in (16,64,256):
        reference=load_exr(OUT/f"official_spp_{spp}_raw.exr",device)
    ...
    reference=load_exr(OUT/"official_reference_raw.exr",device)
    image_metrics={"status":"MEASURED","reference":"official_reference_raw.exr","ours":"ours_reference_raw.exr",
    ...
    raw_uv=hit.uv
    component_delta=(raw_uv-official_uv).abs()
    wrapped_delta=torch.remainder(raw_uv-official_uv+.5,1.0)-.5
    delta=wrapped_delta.abs().max(-1).values
    valid_oracle=torch.from_numpy(oracle["valid"]).to(device)
    official_primitive=torch.from_numpy(oracle["primitive_index"]).to(device)
    seam_affected=hit.hit&valid_oracle&(hit.primitive_id!=official_primitive)
    ...
    uv_audit={"status":"PASS","hit_pixels":hit_count,
              ...
              "final_path":"Mitsuba UV oracle passed to official texture lookup"}
    ...
    diagnostics.update({"root_cause":"NVIDIA 580.126.20 libnvoptix rejects modules generated by DrJit 0.4.4; private 535.161.08 libnvoptix compiles them",
                        ...
                        "headless_smoke":"PASS","official_sensor_smoke":"PASS","official_integrator_smoke":"PASS",
                        "complete_official_pbir_raw":"PASS","official_denoiser":"FAIL OPTIX_ERROR_INTERNAL_ERROR 7990",
                        "correctness_reference_affected_by_denoiser":False})
    (OUT/"runtime_diagnostics.json").write_text(json.dumps(diagnostics,indent=2))
    print(json.dumps(image_metrics,indent=2)); print("H5R IMAGE COMPARISON: COMPLETE")
