"""Spatial-GMM/vMF importance guiding for environment-light sampling."""
...
class SpatialVmfMixture:
    """A 64-component spatial proposal for directional importance sampling."""
    ...
    def __init__(self,position:torch.Tensor,std:torch.Tensor,weight:torch.Tensor) -> None:
        if position.shape!=(64,3) or std.shape!=(64,) or weight.shape!=(64,):
            raise ValueError("the spatial proposal must contain 64 isotropic components")
    ...
    @classmethod
    def from_checkpoint(cls,path:Path,device:torch.device) -> "SpatialVmfMixture":
    ...
    def sample(self,points:torch.Tensor,uniforms:torch.Tensor) -> VmfSamples:
        ...
        # Component selection and the polar sample reuse the remapped variate.
