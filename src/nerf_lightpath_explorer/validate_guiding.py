"""Export the spatial GMM/vMF proposal and per-surface diagnostics."""
...
from nerf_lightpath_explorer.pbir.demo_renderer import center_camera_rays
...
OUT=RESULTS_ROOT/"demo"; PC=OUT/"light_point_cloud"


def main() -> None:
    OUT.mkdir(parents=True,exist_ok=True); PC.mkdir(exist_ok=True)
    emitter=load_emitter(); device=emitter.device; vmf=SpatialVmfMixture.from_checkpoint(CHECKPOINT_PATH,device)
