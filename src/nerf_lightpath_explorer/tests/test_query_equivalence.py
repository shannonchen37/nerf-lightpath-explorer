...
from nerf_lightpath_explorer.tests._common import PROJECT_ROOT, deterministic_rays, load_emitter
...
def expected_reference(emitter, origins, directions, camera_idx, near):
...
def main() -> None:
    ...
    for ray_count in (1, 8, 128):
        ...
        expected_rgb = expected_reference(emitter, origins, directions, 0, near)
        query_rgb = emitter.query_radiance(origins, directions, camera_idx=0, near=near)
        metrics = error_metrics(expected_rgb, query_rgb)
        all_metrics[f"expected_equivalence_n{ray_count}"] = metrics
        print(f"ray count: {ray_count}")
        print(f"expected RGB: {expected_rgb[:min(ray_count, 3)].cpu().tolist()}")
        print(f"query RGB: {query_rgb[:min(ray_count, 3)].cpu().tolist()}")
    ...
    tolerance = 1e-5
    pass_equivalence = all(v["max_abs_error"] <= tolerance for k, v in all_metrics.items() if k.startswith("expected_"))
