"""Local causal explorer server with cached exact traces and interventions."""
...
ROOT=Path(__file__).resolve().parents[3]; EXPLORER_RESULTS=ROOT/"results/causal_explorer"; TRACES=EXPLORER_RESULTS/"traces"; INTERVENTIONS=EXPLORER_RESULTS/"interventions"
DEFAULT_REMOTE_ENV="export CUDA_VISIBLE_DEVICES=1 MI_DEFAULT_VARIANT=cuda_ad_rgb PYTHONPATH=src"
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self,path):
        ...
        if not clean: clean="src/nerf_lightpath_explorer/causal_explorer/index.html"
        ...
        try:
            if p.path=="/api/trace":
                ...
                if not target.exists():
                    self.remote_run(f"{shlex.quote(self.remote_python)} src/nerf_lightpath_explorer/build_trace_presets.py --pixels {x},{y} && {shlex.quote(self.remote_python)} src/nerf_lightpath_explorer/validate_attribution.py")
                    self.sync(f"results/causal_explorer/traces/{target.name}",target)
            ...
            if p.path=="/api/intervention":
                ...
                if not target.exists():
                    ...
                    self.remote_run(f"{shlex.quote(self.remote_python)} src/nerf_lightpath_explorer/intervene_pixel.py {args}")
                    self.sync(f"results/causal_explorer/interventions/{name}",target)
                data=target.read_bytes(); self.send_response(200); self.send_header("Content-Type","application/json"); self.send_header("Content-Length",str(len(data))); self.end_headers(); self.wfile.write(data); return
            if p.path=="/api/sampler":
                self.json_response(json.loads((ROOT/"results/demo/sampling_ab/sampling_ab_metrics.json").read_text())); return
            if p.path=="/api/gmm":
                self.json_response(json.loads((ROOT/"results/demo/gmm_components.json").read_text())); return
...
def main():
    ...
    p.add_argument("--remote",default=os.environ.get("LIGHTPATH_REMOTE"))
    p.add_argument("--remote-root",default=os.environ.get("LIGHTPATH_REMOTE_ROOT"))
    p.add_argument("--remote-python",default=os.environ.get("LIGHTPATH_REMOTE_PYTHON","python"))
    p.add_argument("--remote-env",default=os.environ.get("LIGHTPATH_REMOTE_ENV",DEFAULT_REMOTE_ENV))
    ...
    print(f"Causal explorer: http://127.0.0.1:{a.port}"); ThreadingHTTPServer(("127.0.0.1",a.port),Handler).serve_forever()
